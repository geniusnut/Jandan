...
import com.alensw.support.thread.*;
 ...
 */
public class GifMovie extends Picture {
	...
	private int mWidth;
	private int mHeight;
	protected static ThreadPoolExecutor mExecutor = new AsyncExecutor(1, 2, 1, BaseThread.THREAD_PRIORITY_LOAD);
	...
	public GifMovie(int gif, Uri uri, Handler handler, Bitmap bitmap) {
		super(bitmap, TYPE_MOVIE);
		super.mMimeType = "image/gif";
		super.mUri = uri;
		init(gif, handler);
	}
	...
	private boolean drawFrame(int index, int buffer, int hash) {
		...
		synchronized (this) {
			try {
				if (mGifImage != 0 && buffer != 0 && hash == mGifImage && getBitmap() != null)
					ret = JniUtils.gifDrawFrame(mGifImage, index, buffer, getBitmap());
			} catch (Throwable e) {
