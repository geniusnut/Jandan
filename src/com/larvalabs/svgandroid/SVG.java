 ...
 */
public class SVG {
	private static SparseArray<WeakReference<SVG>> mCache = new SparseArray<WeakReference<SVG>>(4);
	...
	private static SVG getFromCache(int resId) {
		synchronized (mCache) {
			final WeakReference<SVG> ref = mCache.get(resId);
	...
	private static void putToCache(int resId, SVG svg) {
		synchronized (mCache) {
			mCache.put(resId, new WeakReference<SVG>(svg));
		}
	}
